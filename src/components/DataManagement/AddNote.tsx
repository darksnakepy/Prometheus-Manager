    ...
    <div className="flex flex-row justify-center z-15">
      <div className="w-[50%] bg-[#1c1f20] absolute top-[10%] flex-col border-2 border-[#1545af] rounded-2xl ml-auto mr-auto">
        <div className="flex items-center flex-col gap-25px overflow-hidden">
          <h1 className="text-white text-24 font-bold mt-8">Create a secure note</h1>
          <div className="flex flex-col w-[40%] mt-2">
            <label className="text-white text-sm mb-1" htmlFor="header">Title of the note</label>
            <input type="text" id="header" onChange={(e: ChangeEvent<HTMLInputElement>) => setHeader(e.target.value)} className="outline-none pl-3 bg-[#181a1b] h-12 mb-[5%] rounded-[8px] placeholder-white-[0.6] placeholder-14px text-14px text-white" placeholder="Note's header" />
          </div>
          ...
          <div className="flex w-[40%] justify-between mt-5 mr-9.5">
            <button onClick={submitNotes} className="flex-none w-[27%] min-w-fit h-10 mt-5 text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-[#1545af] dark:hover:bg-blue-800 focus:outline-none">Add note</button>
